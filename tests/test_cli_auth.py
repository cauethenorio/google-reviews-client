...
class TestFindTokensFiles:
    def test_explicit_path_exists(self, tmp_cwd):
        f = tmp_cwd / "my-tokens.json"
        f.write_text("{}")
        result = find_tokens_files(tmp_cwd, explicit_path=f)
        assert result == f

    def test_explicit_path_not_found(self, tmp_cwd):
        with pytest.raises(FileNotFoundError):
            find_tokens_files(tmp_cwd, explicit_path=tmp_cwd / "nope.json")

    def test_auto_detect_single(self, tmp_cwd):
        f = tmp_cwd / "credentials.abc123.json"
        f.write_text("{}")
        result = find_tokens_files(tmp_cwd)
        assert result == f

    def test_auto_detect_none(self, tmp_cwd):
        with pytest.raises(NoFilesFoundError):
            find_tokens_files(tmp_cwd)

    def test_auto_detect_multiple(self, tmp_cwd):
        (tmp_cwd / "credentials.abc.json").write_text("{}")
        (tmp_cwd / "credentials.def.json").write_text("{}")
