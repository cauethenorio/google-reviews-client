...
1. **A Google Business Profile** — a verified business listing on Google, active for at least 60 days
2. **A Google Cloud project** — this is where your API credentials live
3. **API access approval** — Google requires a formal application to use the Business Profile APIs
4. **APIs enabled** — the specific Google APIs this project uses
5. **OAuth consent screen** — configured so you can authenticate
6. **OAuth credentials** — a downloaded JSON file the CLI uses to authenticate you

If you already have all of these, skip to [Installation](#installation). Otherwise, follow the steps below — they walk you through everything.
...
**If your business is already on Google Maps** but you haven't claimed it, search for it on [Google Maps](https://maps.google.com), click your business, and select **Claim this business**.
...
> Applications are typically reviewed within 14 days. You can check your status in Cloud Console → APIs → Quotas for the Account Management API: **0 requests per minute** means pending, **300 requests per minute** means approved.

### 4. Enable the required APIs

Once your application is approved, you need to turn on the APIs in your Cloud project.

Go to the [API Library](https://console.cloud.google.com/apis/library) and search for and enable each of these:
...
The consent screen is what users see when they authorize your app. Even if you're the only user, Google requires this.
...
This creates the JSON file that the CLI uses to start the authentication flow.
...
> The downloaded file will be named something like `client_secret_123456.apps.googleusercontent.com.json` — the CLI auto-detects it, so you don't need to rename it.
...
On first run, a browser window opens for Google OAuth. After granting consent, the CLI saves your tokens locally (e.g., `credentials.user@gmail.com.json`) so subsequent runs don't require re-authentication.

You can authenticate multiple Google accounts — each gets its own tokens file. If the CLI finds multiple tokens files, it will ask you to specify which one to use.
...
On subsequent runs, if a `reviews-{location_id}.jsonl` file already exists, the CLI automatically syncs — fetching only new or updated reviews and appending them to the file.
